---
title: "Part 2: Basic text analysis using collocation, keyness and dictionary"
...
By collocation analysis, we can identify multi-word expressions that are very frequent in newspapers articles. One of the most common type of multi-word expressions is proper names, which can be identifed simply based on capitalization in English texts.
...
Keyness is a statistical measure originally implemented in [WordSmith](http://www.lexically.net/wordsmith/) to discover frequent words in target documents. This statistic is essentially a signed chi-squre, where words more frequent than expected are given positive sign. 
...
Sentiment analysis is one of the most popular quantitative text analysis methods. You can use `tokens_lookup()` or `dfm_looup()` to perform sentiment analysis. **quanteda** contains Lexicoder Sentiment Dictionary created by Young and Soroka.
